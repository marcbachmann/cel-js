...
function benchmarkEvaluation() {
  ...
  for (const test of TEST_EXPRESSIONS) {
    ...
    let parseSupported = {local: true, package: true}
...
function displaySummary(parseResults, evalResults, combinedResults) {
  ...
  const parseStats = calculateStats(parseResults)
  const evalStats = calculateStats(evalResults)
  const combinedStats = calculateStats(combinedResults)
