...
test('Environment - basic usage', () => {
  const env = new Environment()
  const result = env.evaluate('1 + 2')
  assert.strictEqual(result, 3n)
})

test('Environment - variable registration and type checking', () => {
  const env = new Environment()
...
test('Environment - context variables without explicit type get dyn', () => {
...
test('Environment - custom function registration', () => {
  const env = new Environment()
...
test('Environment - chaining methods', () => {
  const env = new Environment()
...
test('Environment - evaluate with string expression', () => {
...
test('Environment - complex expression with multiple features', () => {
  const env = new Environment()
...
test('Environment - error handling with context', () => {
...
test('Environment - function overloads', (t) => {
  const env = new Environment()
...
test('Environment - inheritance from global functions', () => {
  const env = new Environment()
...
test('Environment - mixed built-in and custom functions', () => {
  const env = new Environment()
    .registerFunction('multiplyBy2(int): int', (x) => x * 2n)
...
test('Environment - variable type validation', () => {
  const env = new Environment().registerVariable('count', 'int').registerVariable('name', 'string')
...
test('Environment - empty context', () => {
  const env = new Environment()
...
test('Environment - no context parameter', () => {
  const env = new Environment()
...
test('Environment - custom operator registration with Vector types', () => {
  class Vec2 {
    constructor(x, y) {
      this.x = x
      this.y = y
...
test('Environment - parse() method for AST reuse', () => {
  const env = new Environment().registerVariable('x', 'int')
...
test('Environment - unlistedVariablesAreDyn with missing variable', () => {
...
test('Environment - unlistedVariablesAreDyn with registered variable', () => {
...
test('Environment - operator overloading with mixed types', () => {
  const env = new Environment()
