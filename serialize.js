  ...
  switch (op) {
    ...
    case '-':
      ...
      if (args.length === 1) {
        ...
        if (Array.isArray(args[0]) && ['+', '-', '*', '/', '%'].includes(args[0][0])) {
...
function needsParentheses(ast, parentOp) {
  ...
  if ((parentOp === '*' || parentOp === '/') && ast[0] === '-' && ast.length === 2) {
    return false
  }
  ...
  if (
    ...
    if (ast.length > 2 && Array.isArray(ast[1]) && Array.isArray(ast[2])) {
      return true
    }
  }
  ...
  if ((parentOp === '!' || parentOp === '-') && ast.length === 2) {
...
function wrapIfNeeded(ast, parentOp) {
  ...
  if (Array.isArray(ast) && ast[0] === '/' && parentOp === undefined && ast.length === 3) {
    const [_, leftExpr, rightExpr] = ast

    // Check if we need to preserve explicit parentheses for nested expressions
    if (
      Array.isArray(leftExpr) &&
      leftExpr[0] === '*' &&
      leftExpr.length === 3 &&
