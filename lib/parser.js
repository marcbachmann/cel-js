...
export class Parser {
  ...
  parseExpression() {
    ...
    if (this.match(TOKEN.QUESTION)) {
      ...
      return new Node(token.pos, this.input, ['?:', expr, consequent, this.parseExpression()])
    }
  ...
  parseLogicalOr() {
    ...
    while (this.match(TOKEN.OR)) {
      ...
      expr = new Node(token.pos, this.input, [token.value, expr, this.parseLogicalAnd()])
    }
  ...
  parseLogicalAnd() {
    ...
    while (this.match(TOKEN.AND)) {
      ...
      expr = new Node(token.pos, this.input, [token.value, expr, this.parseEquality()])
    }
  ...
  parseEquality() {
    ...
    while (this.match(TOKEN.EQ) || this.match(TOKEN.NE)) {
      const token = this.currentToken
      this.currentToken = this.lexer.nextToken()
      expr = new Node(token.pos, this.input, [token.value, expr, this.parseRelational()])
    }
  ...
  parseRelational() {
    ...
    while (
      ...
      const token = this.currentToken
      this.currentToken = this.lexer.nextToken()
      expr = new Node(token.pos, this.input, [token.value, expr, this.parseAdditive()])
    }
  ...
  parseAdditive() {
    ...
    while (this.match(TOKEN.PLUS) || this.match(TOKEN.MINUS)) {
      const token = this.currentToken
      this.currentToken = this.lexer.nextToken()
      expr = new Node(token.pos, this.input, [token.value, expr, this.parseMultiplicative()])
    }
  ...
  parseMultiplicative() {
    ...
    while (this.match(TOKEN.MULTIPLY) || this.match(TOKEN.DIVIDE) || this.match(TOKEN.MODULO)) {
      const token = this.currentToken
      this.currentToken = this.lexer.nextToken()
      expr = new Node(token.pos, this.input, [token.value, expr, this.parseUnary()])
    }
  ...
  parseUnary() {
    ...
    if (token.type === TOKEN.NOT) {
      this.currentToken = this.lexer.nextToken()
      return new Node(token.pos, this.input, ['!_', this.parseUnary()])
    }
    ...
    while (true) {
      if (this.match(TOKEN.DOT)) {
        ...
        if (this.match(TOKEN.LPAREN)) {
          ...
          expr = new Node(property.pos, this.input, ['rcall', property.value, expr, args])
        } else {
          expr = new Node(property.pos, this.input, ['.', expr, property.value])
        }
      } else if (this.match(TOKEN.LBRACKET)) {
        const token = this.consume(TOKEN.LBRACKET)
        const index = this.parseExpression()
        this.consume(TOKEN.RBRACKET)
        expr = new Node(token.pos, this.input, ['[]', expr, index])
      } else {
        break
      }
    }
    ...
    switch (this.currentToken.type) {
      ...
      case TOKEN.IDENTIFIER: {
        ...
        if (this.match(TOKEN.LPAREN)) {
          ...
          const args = this.parseArgumentList()
          this.consume(TOKEN.RPAREN)
          return new Node(identifier.pos, this.input, ['call', identifier.value, args])
        }

        return new Node(identifier.pos, this.input, ['id', identifier.value])
      }
      case TOKEN.LPAREN: {
        this.consume(TOKEN.LPAREN)
  ...
  parseList() {
    ...
    this.consume(TOKEN.RBRACKET)
    return new Node(token.pos, this.input, ['list', elements])
  }
  ...
  parseMap() {
    ...
    return new Node(token.pos, this.input, ['map', properties])
  }
