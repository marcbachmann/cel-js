...
const OP_MACRO_RCALL = {
  name: 'rcall',
  check(chk, _ast, ctx) {
    const m = this.macro
    if (m.callAst) return chk.check(m.callAst, ctx)
    return m.typeCheck(chk, m, ctx)
  },
  evaluate(ev, _ast, ctx) {
    const m = this.macro
    if (m.callAst) return ev.eval(m.callAst, ctx)
    return m.evaluate(ev, m, ctx)
  }
}

const OP_MACRO_CALL = {...OP_MACRO_RCALL, name: 'call'}
...
export class Parser {
  ...
  #expandMacro(pos, op, args) {
    ...
    if (!decl) return this.#node(pos, op, args)
    const ast = this.#node(pos, receiver ? OP_MACRO_RCALL : OP_MACRO_CALL, args)
    ast.macro = decl.handler({ast, args: fnArgs, receiver, methodName, parser: this})
    return ast
  }
