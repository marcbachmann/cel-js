...
export class ASTNode {
  #input
  #pos
  constructor(pos, input, op, args) {
    const opObj = OPERATORS[op]
    this.#input = input
    this.#pos = pos
    this.op = op
    this.check = opObj.check
    this.evaluate = opObj.evaluate
    this.args = args
  }
  ...
  nextToken() {
    while (true) {
      ...
      switch (ch) {
        ...
        case '<':
          if (input[pos + 1] === '=') return {type: TOKEN.LE, value: '<=', pos: (this.pos += 2) - 2}
          return {type: TOKEN.LT, value: '<', pos: this.pos++}
        case '>':
          if (input[pos + 1] === '=') return {type: TOKEN.GE, value: '>=', pos: (this.pos += 2) - 2}
          return {type: TOKEN.GT, value: '>', pos: this.pos++}
        case '!':
          if (input[pos + 1] === '=') return {type: TOKEN.NE, value: '!=', pos: (this.pos += 2) - 2}
...
export class Parser {
  ...
  #node(pos, op, args) {
    const node = new ASTNode(pos, this.input, op, args)
