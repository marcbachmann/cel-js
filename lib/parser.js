...
class Lexer {
  ...
  _readExponent(input, length, pos) {
    ...
    if (ch === 'e' || ch === 'E') {
      ...
      if (ch === '-' || ch === '+') pos++
      const start = pos
      while (pos < length && (ch = input[pos]) <= '9' && ch >= '0') pos++
  ...
  readNumber() {
    ...
    if (input[pos] === '0' && (input[pos + 1] === 'x' || input[pos + 1] === 'X')) {
      ...
      this.pos = pos
      return this._parseAsBigInt(start, pos, true, input[pos])
    }
  ...
  readString(prefix) {
    ...
    const delimiter = input[this.pos++]

    // Check for triple quotes
    if (input[this.pos] === delimiter && input[this.pos + 1] === delimiter) {
  ...
  readTripleQuotedString(delimiter, prefix) {
    ...
    let pos = this.pos
    let ch
    while (pos + 2 < length && (ch = input[pos])) {
      // Check for closing triple delimiter
      if (ch === delimiter && input[pos + 1] === delimiter && input[pos + 2] === delimiter) {
        const rawValue = input.slice(start, pos)
        this.pos = pos + 3 // Skip closing triple quotes
        return this._closeQuotedString(rawValue, prefix)
      }
    ...
    throw new ParseError('Unterminated triple-quoted string', {pos: start - 3, input})
  }
  ...
  readIdentifier() {
    const {pos: start, input, length} = this
    let pos = start
    while (pos < length && this._isIdentifierCharCode(input[pos].charCodeAt(0))) pos++
    const text = input.substring(start, (this.pos = pos))
