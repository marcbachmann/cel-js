...
export class ASTNode {
  ...
  static infix(input, pos, op, left, right) {
    return new ASTNode(input, pos, left.start, right.end, op, [left, right])
  }

  static ternary(input, pos, op, expression, consequent, alternate) {
    return new ASTNode(input, pos, expression.start, alternate.end, op, [expression, consequent, alternate])
  }

  static unary(input, pos, op, arg) {
    return new ASTNode(input, pos, pos, arg.end, op, arg)
  }

  static access(input, pos, op, left, right, end) {
    return new ASTNode(input, pos, left.start, end, op, [left, right])
  }
...
class Lexer {
  ...
  processEscapes(str, isBytes, offset) {
    ...
    let result = ''
    let i = 0
    const fail = (code, message, start, end = start + 1) => {
      throw parseError(code, message, {input, pos: start, start, end: Math.min(end, inputEnd)})
    }
    while (i < str.length) {
      ...
      if (STRING_ESCAPES[next]) {
        ...
        if (isBytes) fail('bytes_unicode_escape', '\\u not allowed in bytes literals', offset + i, offset + i + 2)
        const escapeStart = i
        i += 6
        const hex = str.substring(escapeStart + 2, i)
        if (!/^[0-9a-fA-F]{4}$/.test(hex)) {
          fail('invalid_unicode_escape', `Invalid Unicode escape: \\u${hex}`, offset + escapeStart, offset + i)
        }
        const c = Number.parseInt(hex, 16)
        if (c >= 0xd800 && c <= 0xdfff) {
          fail(
            'invalid_unicode_surrogate',
            `Invalid Unicode surrogate: \\u${hex}`,
            offset + escapeStart,
            offset + i
          )
        }
        ...
        if (!/^[0-9a-fA-F]{2}$/.test(h)) {
          fail('invalid_hex_escape', `Invalid hex escape: \\${next}${h}`, offset + escapeStart, offset + i)
        }
        ...
        if (!/^[0-7]{3}$/.test(o)) {
          fail('invalid_octal_escape', 'Octal escape must be 3 digits', offset + escapeStart, offset + i)
        }
        const value = Number.parseInt(o, 8)
        if (value > 0xff) {
          fail('octal_escape_out_of_range', `Octal escape out of range: \\${o}`, offset + escapeStart, offset + i)
        }
        result += String.fromCharCode(value)
      } else {
        fail('invalid_escape_sequence', `Invalid escape sequence: \\${next}`, offset + i, offset + i + 2)
      }
    }
