...
export function registerOverloads(registry) {
  ...
  binaryOverload('list<A>', '+', 'list<A>', (a, b) => [...a, ...b])
  ...
  binaryOverload('A', 'in', 'list<A>', listMembership)
  binaryOverload('A', 'in', 'map<A, B>', mapIncludes)
  ...
  binaryOverload('list<A>', `==`, 'list<A>', (a, b, ast, ev) => {
  ...
  binaryOverload('map<A, B>', `==`, 'map<A, B>', (a, b, ast, ev) => {
    if (a instanceof Map && b instanceof Map) {
      if (a.size !== b.size) return false
      for (const [key, value] of a)
        if (!(b.has(key) && isEqual(value, b.get(key), ev))) return false
      return true
    }
...
function isEqual(a, b, ev) {
  ...
  switch (typeof a) {
    ...
    case 'object':
      ...
      return overload.handler(a, b, null, ev)
  }
