...
export function registerOverloads(registry) {
  ...
  binaryOverload('int', '/', 'int', (a, b, ast) => {
    if (b === 0n) throw evaluationError('division_by_zero', 'division by zero', ast)
    return a / b
  })
  binaryOverload('int', '%', 'int', (a, b, ast) => {
    if (b === 0n) throw evaluationError('modulo_by_zero', 'modulo by zero', ast)
    return a % b
  })
  ...
  binaryOverload('uint', '/', 'uint', (a, b, ast) => {
    if (b.valueOf() === 0n) throw evaluationError('division_by_zero', 'division by zero', ast)
    return new UnsignedInt(a.valueOf() / b.valueOf())
  })
  binaryOverload('uint', '%', 'uint', (a, b, ast) => {
    if (b.valueOf() === 0n) throw evaluationError('modulo_by_zero', 'modulo by zero', ast)
    return new UnsignedInt(a.valueOf() % b.valueOf())
  })
