...
const unaryOverload = (...args) => globalRegistry.unaryOverload(...args)
const binaryOverload = (...args) => globalRegistry.binaryOverload(...args)
...
function listIncludes(value, list, ast, ev) {
  ...
  for (const v of list) if (isEqual(value, v, ev)) return true
  return false
}
...
binaryOverload('list', `==`, 'list', (a, b, ast, ev) => {
  if (Array.isArray(a) && Array.isArray(b)) {
    const length = a.length
    if (length !== b.length) return false
    for (let i = 0; i < length; i++) {
      if (!isEqual(a[i], b[i], ev)) return false
    }
    return true
  }
...
binaryOverload('map', `==`, 'map', (a, b, ast, ev) => {
  if (a instanceof Map && b instanceof Map) {
    if (a.size !== b.size) return false
    for (const [key, value] of a) if (!(b.has(key) && isEqual(value, b.get(key), ev))) return false
    return true
  }
  ...
  if (a instanceof Map || b instanceof Map) {
    ...
    for (const [key, value] of map) {
      if (!(key in obj && isEqual(value, obj[key], ev))) return false
    }
    return true
  }
  ...
  for (let i = 0; i < keysA.length; i++) {
    const key = keysA[i]
    if (!(key in b && isEqual(a[key], b[key], ev))) return false
  }
  return true
})
...
function isEqual(a, b, ev) {
  ...
  switch (typeof a) {
    ...
    case 'object':
      ...
      const leftType = ev.objectTypesByConstructor.get(a.constructor)
      const rightType = ev.objectTypesByConstructor.get(b.constructor)
      if (!leftType || leftType !== rightType) return false
      const handler = ev.overloads['=='][leftType.name]?.[rightType.name]?.handler
