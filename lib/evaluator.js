...
const handlers = new Map(
  Object.entries({
    ...
    '.'(ast, ev, ctx) {
      return withPromise(ast, ev, ev.eval(ast[1], ctx), ast[2], fieldAccess)
    },
    '[]'(ast, ev, ctx) {
      return withPromise(ast, ev, ev.eval(ast[1], ctx), ev.eval(ast[2], ctx), fieldAccess)
    },
    rcall(ast, ev, ctx) {
    ...
    list(ast, ev, ctx) {
      return resolveAstArray(ev, ctx, ast[1])
    },
    map(ast, ev, ctx) {
      return resolveAstEntries(ev, ctx, ast[1])
    },
    '?:'(ast, ev, ctx) {
      return withPromise(ast, ev, ev.eval(ast[1], ctx), ctx, _ternary)
    },
    '!_': unaryOverload,
    '-_': unaryOverload,
    '!=': binaryOverload,
...
function _ternary(ast, ev, condition, ctx) {
...
function _unaryOverload(ast, ev, left) {
...
function _binaryOverload(ast, ev, left, right) {
...
function unaryOverload(ast, ev, ctx) {
  return withPromise(ast, ev, ev.eval(ast[1], ctx), null, _unaryOverload)
}

function binaryOverload(ast, ev, ctx) {
  return withPromise(ast, ev, ev.eval(ast[1], ctx), ev.eval(ast[2], ctx), _binaryOverload)
}
...
function _receiverCall(ast, ev, receiver, args) {
  ...
  const functionCandidates = (ast.functionCandidates ??= ev.registry.getFunctionCandidates(
    receiverAst.checkedType || celTypes.dyn,
    functionName,
    argAst.length
  ))
...
class Evaluator {
  ...
  constructor(opts) {
    ...
    this.stringType = celTypes.string
    this.intType = celTypes.int
    this.doubleType = celTypes.double
    this.boolType = celTypes.bool
    this.nullType = celTypes.null
    this.listType = celTypes.list
    this.mapType = celTypes.map
  }
  ...
  eval(ast, ctx) {
    return (this.handlers.get(ast[0]) || unknownOperationError(ast))(ast, this, ctx)
  }
}
