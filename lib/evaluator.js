...
const handlers = new Map(
  Object.entries({
    ...
    id(ast, s) {
      const key = ast[1]
      const variableDecl = s.variables.get(key)
    ...
    '.'(ast, s) {
      const left = s.eval(ast[1])
      const right = ast[2]
      const value = objectGet(left, right, s.objectTypesByConstructor)
...
class VariablesOverlay {
  ...
  get(name) {
    if (name === this.#decl.name) return this.#decl
    return this.#parent.get(name)
  }
}
...
class PredicateEvaluator extends Evaluator {
  constructor(e, functionName, ast) {
    ...
    this.variables = new VariablesOverlay(this.variables, {
      name: this.predicateVariable,
      type: 'dyn'
    })
