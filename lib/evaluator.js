...
function computeDynamicOverload(ev, ast, leftType, rightType) {
  ...
  return (
    ...
    (leftIsDyn && op.get(ev.celTypes.dyn)?.get(rightType)) ||
    (rightIsDyn && op.get(leftType)?.get(ev.celTypes.dyn)) ||
    (leftIsDyn && rightIsDyn && op.get(ev.celTypes.dyn)?.get(ev.celTypes.dyn)) ||
    false
  )
}
