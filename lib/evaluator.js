  ...
  Object.entries({
    ...
    rcall(ast, s) {
      ...
      try {
        ...
        if (isMacro) {
          ...
          if (fn?.handler) return fn.handler(receiver, ...args)
        }
        throw new EvaluationError(
          `found no matching overload for '${receiverType}.${functionName}(${args
            .map(isMacro ? () => 'ast' : debugType)
            .join(', ')})'`,
          ast
        )
      } catch (err) {
    ...
    call(ast, s) {
      ...
      try {
        ...
        if (isMacro) {
          fn = resolveMacroFunction(fn, args)
          if (fn?.handler) return fn.handler.call(s, ...args)
        } else {
          args = args.map((arg) => s.eval(arg))
          fn = resolveFunction(fn, args)
          if (fn?.handler) return fn.handler.apply(fn.handler, args)
        }
...
function resolveFunction(fn, args) {
  ...
  switch (args.length) {
    case 0:
      return fn
    case 1:
      return fn[debugType(args[0])]
    case 2:
      return fn[debugType(args[0])]?.[debugType(args[1])]
    case 3:
      return fn[debugType(args[0])]?.[debugType(args[1])]?.[debugType(args[2])]
  }
}
