...
function _receiverCall(ast, ev, receiver, args) {
  ...
  const functionCandidates = (ast.functionCandidates ??= ev.registry.getFunctionCandidates(
    receiverAst.checkedType || ev.celTypes.dyn,
    functionName,
    argAst.length
  ))

  const receiverType = ev.debugRuntimeType(receiver, receiverAst.checkedType || ev.celTypes.dyn)
...
class Environment {
  ...
  #evaluateAST(ast, context = null) {
    const ctx = this.#rootContext.fork(context)
