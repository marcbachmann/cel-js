...
operatorOverload('string in list', (a, b) => (Array.isArray(b) ? b.includes(a) : b.has(a)))
for (const t of ['bool', 'int', 'double', 'null', 'list']) {
  operatorOverload(`${t} in list`, listIncludes)
}
...
const supportsStrictEquality = ['type', 'null', 'bool', 'string', 'int', 'double']
for (const t of supportsStrictEquality) {
  operatorOverload(`${t} == ${t}`, strictEqual)
  operatorOverload(`${t} != ${t}`, strictNotEqual)
  operatorOverload(`dyn == ${t}`, strictEqual)
  operatorOverload(`dyn != ${t}`, strictNotEqual)
  operatorOverload(`${t} == dyn`, strictEqual)
  operatorOverload(`${t} != dyn`, strictNotEqual)
}
...
for (const [left, right] of [
  ...
  operatorOverload(`${left} == ${right}`, (a, b) => a == b) // eslint-disable-line eqeqeq
  operatorOverload(`${left} != ${right}`, (a, b) => a != b) // eslint-disable-line eqeqeq
}
...
for (const t of ['bytes', 'list', 'map']) {
  ...
  operatorOverload(`dyn == ${t}`, isEqual)
  operatorOverload(`${t} == dyn`, isEqual)
  operatorOverload(`dyn != ${t}`, (a, b) => !isEqual(a, b))
  operatorOverload(`${t} != dyn`, (a, b) => !isEqual(a, b))
}
...
function getOverload(s, ast, leftType, rightType) {
  ...
  cache[rightType] = overload || false
  return overload
}
...
function computeDynamicOverload(s, ast, leftType, rightType) {
  ...
  if (!leftDynamic && !rightDynamic) return null

  if (leftDynamic && rightDynamic) {
    const overload = op[`dyn<${leftType}>`]?.[`dyn<${rightType}>`]
    if (overload) return overload
  }

  if (leftDynamic) {
    const overload = op[`dyn<${leftType}>`]?.[rightType]
    if (overload) return overload
  }

  if (rightDynamic) {
    const overload = op[leftType]?.[`dyn<${rightType}>`]
    if (overload) return overload
  }

  if (leftDynamic) {
    const overload = op['dyn']?.[rightType]
    if (overload) return overload
  }

  if (rightDynamic) {
    const overload = op[leftType]?.['dyn']
    if (overload) return overload
  }
