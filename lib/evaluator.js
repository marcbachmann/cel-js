...
const handlers = new Map(
  Object.entries({
    ...
    id(ast, ev) {
      ...
      if (variableDecl.type === type || variableDecl.type === 'dyn') return val

      throw new EvaluationError(
        `Variable '${key}' is not of type '${variableDecl.type}', got '${type}'`,
        ast
      )
    },
    '||'(ast, ev) {
    ...
    '[]'(ast, ev) {
      ...
      if (Array.isArray(left)) {
        if (!(typeof right === 'number' || typeof right === 'bigint')) {
          throw new EvaluationError(`No such key: ${right} (${ev.debugType(right)})`, ast)
        }
        if (right < 0) {
    ...
    rcall(ast, ev) {
      ...
      let fn = ev.getFunction(functionName, receiverType)
      if (!fn) {
        throw new EvaluationError(
      ...
      try {
        if (fn.macro) return fn.handler.call(ev, receiver, ast)
        return fn.handler.call(ev, receiver, ...args)
      } catch (err) {
    ...
    call(ast, ev) {
      const functionName = ast[1]
      let fn = ev.getFunction(functionName)
      ...
      try {
        if (fn.macro) return fn.handler.call(ev, ast)
        return fn.handler.apply(ev, args)
      } catch (err) {
        if (err instanceof EvaluationError) throw err.withAst(ast)
        throw err
      }
  ...
  #getFunctionWithLegacyFunctions(name, type) {
    if (this.#legacyFunctions) {
      ...
      if (typeof fn === 'function')
        return {unknownargs: true, handler: fn, returnType: this.#registry.getType('dyn')}
    }
...
class Evaluator {
  ...
  debugType(v) {
    switch (typeof v) {
      ...
      case 'object':
        ...
        return (
          this.objectTypesByConstructor.get(v.constructor)?.name ||
  ...
  isDynamic(ast) {
    ...
    switch (ast[0]) {
      ...
      case 'call': {
        const functionName = ast[1]
        switch (this.functions.returnTypes[functionName]?.type) {
          case 'dyn':
            return true
          case undefined: {
