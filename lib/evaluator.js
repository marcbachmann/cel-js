...
const handlers = new Map(
  Object.entries({
    ...
    id(ast, ev) {
      ...
      if (variableDecl === type || variableDecl.type === 'dyn') return val

      throw new EvaluationError(
        `Variable '${key}' is not of type '${variableDecl.type}', got '${type.type || type.name}'`,
        ast
      )
    },
    '||'(ast, ev) {
    ...
    rcall(ast, ev) {
      ...
      if (!functionCandidates.exists) {
        throw new EvaluationError(
          `Function not found: '${functionName}' for value of type '${
            receiverType.type || receiverType.name
          }'`,
          ast
        )
      }
      ...
      if (byReceiver.hasMacro) {
        argTypes = ast.macroArgTypes ??= ast[3].map(() => ev.registry.getFunctionType('ast'))
      } else {
        args = ast[3].map((arg) => ev.eval(arg))
        argTypes = args.map((arg) => ev.debugType(arg))
      }
      ...
      if (!decl) {
        throw new EvaluationError(
          `found no matching overload for '${receiverType.type}.${functionName}(${argTypes
            .map((t) => t.type)
            .join(', ')})'`,
          ast
        )
    ...
    call(ast, ev) {
      ...
      if (functionCandidates.hasMacro) {
        argTypes = ast.macroArgTypes ??= ast[2].map(() => ev.registry.getFunctionType('ast'))
      } else {
        args = ast[2].map((arg) => ev.eval(arg))
        argTypes = args.map((arg) => ev.debugType(arg))
      }
    ...
    '?:'(ast, ev) {
      ...
      return condition ? ev.eval(ast[2]) : ev.eval(ast[3])
    },
    '!_': unaryOverload,
    '-_': unaryOverload,
    '!=': binaryOverload,
...
function unaryOverload(ast, ev) {
  const left = ev.eval(ast[1])
  const leftType = ev.debugType(left).type
  const overload = ev.overloads[ast[0]]?.[leftType]
...
function getOverload(ev, ast, leftType, rightType) {
...
function computeDynamicOverload(ev, ast, leftType, rightType) {
  ...
  const leftDynamic = ev.isDynamic(ast[1])
  const rightDynamic = ev.isDynamic(ast[2])
...
function binaryOverload(ast, ev) {
  ...
  const right = ev.eval(ast[2])
  const leftType = ev.debugType(left)?.type
  const rightType = ev.debugType(right)?.type
  const overload =
    ev.overloads[ast[0]][leftType]?.[rightType] || getOverload(ev, ast, leftType, rightType)
...
class Environment {
  ...
  #formatTypeForCheck(typeDecl) {
    if (typeDecl.type === 'list' && `${typeDecl}` === `list<dyn>`) return 'list'
    if (typeDecl.type === 'map' && `${typeDecl}` === `map<dyn, dyn>`) return 'map'
    return `${typeDecl}`
  }
...
class Evaluator {
  ...
  debugType(v) {
    switch (typeof v) {
      ...
      case 'bigint':
        return this.registry.getType('int')
      case 'number':
        return this.registry.getType('double')
      case 'boolean':
        return this.registry.getType('bool')
      case 'object':
        if (v === null) return this.registry.getType('null')
        return (
  ...
  isDynamic(ast) {
    ...
    switch (ast[0]) {
      ...
      case 'call': {
        const functionName = ast[1]
        const hasReceiver = ast[0] === 'rcall'
        const returnType = (ast.functionCandidates ??= this.registry.getFunctionCandidates(
          hasReceiver,
          functionName,
          ast[hasReceiver ? 3 : 2].length
        )).returnType
...
class PredicateEvaluator extends Evaluator {
  constructor(e, functionName, ast) {
    ...
    this.variables = new VariablesOverlay(
      this.variables,
      this.predicateVariable,
      this.registry.getType('dyn')
    )
