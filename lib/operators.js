...
function attachFunctionErrorAst(error, ast) {
  if (error?.withAst instanceof Function) return error.withAst(ast)
  return error
}
  ...
  try {
    ...
    if (result instanceof Promise) {
      return result.catch((error) => {
        throw attachFunctionErrorAst(error, ast)
      })
    }
    return result
  } catch (error) {
  ...
  rcall: {
    check(chk, ast, ctx) {
      ...
      if (!receiverType.hasPlaceholderType && !argTypes.some((t) => t.hasDynType)) {
        ...
        const handle = (handler.__handle ??= (a, ev, _ast) => callFunctionHandler(handler, ev, a, _ast))
