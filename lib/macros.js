...
export function registerMacros(registry) {
  ...
  function bindOptionalEvaluate(ev, macro, ctx, boundValue) {
    return ev.eval(
      macro.exp,
      ctx.forkWithVariable(macro.val.checkedType, macro.var).setVariableValue(boundValue)
    )
  }
  ...
  registry.registerFunctionOverload('CelNamespace.bind(ast, dyn, ast): dyn', ({args}) => {
    return {
      ...
      typeCheck(checker, macro, ctx) {
        const newCtx = ctx.forkWithVariable(checker.check(macro.val, ctx), macro.var)
