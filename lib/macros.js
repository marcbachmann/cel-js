...
function createMapExpander(hasFilter) {
  ...
  const predicateLabel = `${functionDesc} filter predicate`

  return ({args, receiver, ast: callAst}) => {
    const transform = hasFilter ? args[2] : args[1]
    let predicate = hasFilter ? args[1] : null
    const accuId = ast(callAst, OPS.accuValue)

    let step = ast(callAst, OPS.accuPush, transform)
    if (predicate) {
      predicate = ast(callAst, OPS.predicateGuard, {expr: predicate, label: predicateLabel})
      step = ast(callAst, OPS.ternary, [predicate, step, accuId])
    }
...
function createFilterExpander() {
  ...
  const invalidMsg = `${functionDesc} invalid predicate iteration variable`
  const label = `${functionDesc} predicate`

  return ({args, receiver, ast: callAst}) => {
    const accuId = ast(callAst, OPS.accuValue)
    const iterVarId = ast(callAst, OPS.id, assertIdentifier(args[0], invalidMsg))
    const validatedPredicate = ast(args[1], OPS.predicateGuard, {expr: args[1], label})
    const appendItem = ast(callAst, OPS.accuPush, iterVarId)
    const step = ast(callAst, OPS.ternary, [validatedPredicate, appendItem, accuId])
...
function createQuantifierExpander(opts) {
  ...
  const label = `${opts.name}(var, predicate) predicate`
  return ({args, receiver, ast: callAst}) => {
    const predicate = ast(args[1], OPS.predicateGuard, {expr: args[1], label})
...
function createHasExpander() {
  ...
  function typeCheck(checker, macro, ctx) {
    ...
    if (!macro.macroHasProps) {
      const props = []
      while ((node.op === '.' || node.op === '.?') && props.push(node)) node = node.args[0]
...
export function registerMacros(registry) {
    ...
    createQuantifierExpander({
      ...
      transform({ast: callAst, predicate, opts}) {
        ...
        return {
          ...
          step: ast(callAst, OPS.ternary, [predicate, ast(callAst, OPS.accuInc), accuId]),
          result: opts.result
        }
      }
    })
  ...
  function bindOptionalEvaluate(ev, macro, ctx, boundValue) {
    return ev.eval(macro.exp, ctx.forkWithVariable(macro.var, macro.val.checkedType, boundValue))
  }
  ...
  registry.registerFunctionOverload('CelNamespace.bind(ast, dyn, ast): dyn', ({args}) => {
    return {
      ...
      typeCheck(checker, macro, ctx) {
        const newCtx = ctx.forkWithVariable(macro.var, checker.check(macro.val, ctx))
        return checker.check(macro.exp, newCtx)
      },
      evaluate(ev, macro, ctx) {
