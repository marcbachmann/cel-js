...
export class TypeChecker {
  ...
  checkCall(ast) {
    ...
    if (!byReceiver.exists) {
      throw new TypeError(`Function not found: '${functionName}'`, ast)
    }

    if (byReceiver.hasMacro) {
      const candidate = byReceiver.functions[0]
      if (candidate.typeCheck) return candidate.typeCheck(this, null, ast[2])
      return candidate.returnType
    }

    const argTypes = ast[2].map((arg) => this.check(arg))
    const argTypeNames = argTypes.map((t) => t.type)
    const decl = byReceiver.findMatch(argTypeNames)
  ...
  checkReceiverCall(ast) {
    ...
    const byReceiver = functionCandidates.filterByReceiverType(receiverTypeName)
    if (byReceiver.hasMacro) {
      const candidate = byReceiver.functions[0]
      if (candidate.typeCheck) return candidate.typeCheck(this, receiverType, ast[3])
      return candidate.returnType
    }

    const argTypes = ast[3].map((arg) => this.check(arg))
    const argTypeNames = argTypes.map((t) => t.type)
    const decl = byReceiver.findMatch(argTypeNames)

    if (!decl) {
      throw new TypeError(
        `found no matching overload for '${receiverTypeName}.${methodName}(${argTypeNames.join(
          ', '
        )})'`,
        ast
      )
