...
export class TypeChecker {
  ...
  checkAccess(ast) {
    ...
    if (ast[0] === '.') {
      ...
      const baseType = leftType.replace(/^(?:list|map)<.+>$/, () => {
        // Extract the base type (list or map) without the generic part
        return leftType.startsWith('list') ? 'list' : 'map'
      })

      if (baseType === 'map' || baseType === 'list' || this.objectTypes.has(leftType)) {
        // Extract value type from map<K, V> for property access
        const mapTypeMatch = leftType.match(/^map<.+,\s*(.+)>$/)
        if (mapTypeMatch) return mapTypeMatch[1]
        return 'dyn'
      }
    ...
    if (ast[0] === '[]') {
      ...
      // Extract element type from list<T> notation
  ...
  checkMethodCall(ast) {
    ...
    if (!['map', 'all', 'exists', 'exists_one', 'filter'].includes(methodName)) {
      return this.functions.returnTypes[methodName] || 'dyn'
    }

    const varAst = ast[3][0]
    const varName = Array.isArray(varAst) && varAst[0] === 'id' ? varAst[1] : null

    const elementTypeMatch = receiverType.match(/^list<(.+)>$/)
    const mapTypeMatch = receiverType.match(/^map<(.+),/)
    let elementType = 'dyn'
    if (elementTypeMatch) elementType = elementTypeMatch[1]
    else if (mapTypeMatch) elementType = mapTypeMatch[1]

    let descriptor = `${elementType}.${methodName}(var, predicate)`
    if (methodName === 'filter') {
      descriptor = `${elementType}.${methodName}(var, filter)`
    } else if (methodName === 'map') {
      descriptor = `${elementType}.${methodName}(var, ${
        ast[3].length === 3 ? `filter, ` : ''
      }transform)`
    }

    if (!varName) {
      throw new TypeError(
        `${descriptor} requires first argument to be an identifier`,
        Array.isArray(varAst) ? varAst : ast
      )
    }

    const prevVarType = this.variables.get(varName)
    this.variables.set(varName, elementType)

    try {
      const verify = (expr, returnType, msg) => {
        const exprType = this.check(expr)
        if (!returnType || exprType === returnType || exprType === 'dyn') return exprType
        throw new TypeError(`${msg} must return ${returnType}, got '${exprType}'`, expr)
      }

      if (methodName !== 'map') {
        verify(ast[3][1], 'bool', `${descriptor} predicate`)
        if (methodName === 'filter') {
      ...
      const [filter, transform] = ast[3].length === 3 ? [ast[3][1], ast[3][2]] : [null, ast[3][1]]
      if (filter !== null) verify(filter, 'bool', `${descriptor} filter predicate`)

      const returnType = verify(transform, null, `${descriptor} transform predicate`)

      if (returnType === 'dyn') return 'list'
      return `list<${returnType}>`
    } finally {
      if (prevVarType !== undefined) this.variables.set(varName, prevVarType)
      else this.variables.delete(varName)
    }
  }
