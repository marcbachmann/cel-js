...
export class TypeChecker {
  constructor({environment, variables, objectTypes, objectTypesByConstructor, registry}) {
  ...
  checkUnaryOperator(ast) {
    ...
    const overload = this.overloads[op]?.get(operandType)
  ...
  checkBinaryOperator(ast) {
    ...
    if (leftType.kind === 'dyn' || rightType.kind === 'dyn') {
      if (
        op === '<' ||
        op === '<=' ||
        op === '>' ||
    ...
    const overload = this.overloads[op]?.get(leftType)?.get(rightType)
    if (overload) return overload.returnType

    throw new TypeError(`no such overload: ${leftType.type} ${ast[0]} ${rightType.type}`, ast)
  }
}
