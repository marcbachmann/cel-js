...
function parseDeclaration(signature, customTypes) {
  ...
  if (returnType && !(types.has(returnType) || customTypes?.has(returnType))) {
    throw new Error(`Invalid return type '${returnType}' in ${signature}`)
  }

  for (let i = 0; i < argTypes.length; i++) {
    if (types.has(argTypes[i]) || customTypes?.has(argTypes[i])) continue
...
export function createFunctionRegistry(localFunctions, customTypes) {
  return {
    functionOverload(s, handler) {
      const declaration = parseDeclaration(s, customTypes)

      let fnTarget = declaration.receiverType
        ? (localFunctions[declaration.receiverType] ??= Object.create(null))
        : localFunctions.standalone

      localFunctions.returnTypes[declaration.name] ??= declaration.returnType
      ...
      for (const argType of declaration.argTypes)
        fnTarget = fnTarget[argType] ??= Object.create(null)
...
const {functionOverload} = createFunctionRegistry(allFunctions)
...
for (const _t in TYPES) {
  ...
  functionOverload(`dyn(${type.name}): ${type.name}`, (v) => v)
...
export function debugType(v) {
  switch (typeof v) {
    ...
    case 'object':
      ...
      return objectTypes.get(v.constructor) || unsupportedType(v.constructor?.name || typeof v)
    default:
