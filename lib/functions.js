...
export function registerFunctions(registry) {
  ...
  functionOverload('size(map): int', (v) =>
    BigInt(v instanceof Map ? v.size : objectKeys(v).length)
  )
  ...
  functionOverload('map.size(): int', (v) =>
    BigInt(v instanceof Map ? v.size : objectKeys(v).length)
  )
  ...
  functionOverload('string.indexOf(string): bool', (string, search) =>
