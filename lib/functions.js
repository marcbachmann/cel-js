...
function hasNestedField(ev, selector, ast) {
  if (selector instanceof ASTNode) {
    switch (selector[0]) {
      case 'id': {
        const obj = ev.ctx.get(selector[1])
        if (obj === undefined)
      ...
      case '.': {
        ...
        return objectGet(obj, selector[2], ev.objectTypesByConstructor)
      }
    }
  }
  throw new EvaluationError('has() invalid argument', ast)
...
functionOverload('dyn(dyn): dyn', (v) => v)
...
export function objectGet(obj, key, objectTypesByConstructor) {
