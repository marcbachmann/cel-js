...
const functionOverload = (sig, handler) => globalRegistry.registerFunctionOverload(sig, handler)
...
function mapTypeCheck(checker, receiverType, args) {
  ...
  return withVariableContext(checker, varName, elementType, () => {
    ...
    const transformedType = checker.check(args[args.length - 1])
    return checker.getType(`list<${transformedType}>`)
  })
}
