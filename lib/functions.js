...
functionOverload('has(ast): bool', function (ast) {
  ...
  if (selector[0] === '.') return hasNestedField(this, selector, ast) !== undefined
  throw new EvaluationError('has() invalid argument', ast)
})

function hasNestedField(self, selector, ast) {
  if (selector instanceof ASTNode) {
    switch (selector[0]) {
      case 'id': {
        const obj = self.ctx.get(selector[1])
        if (obj === undefined)
          throw new EvaluationError(`Unknown variable: ${selector[1]}`, selector)
        return obj
      }
      case '.': {
        const obj = hasNestedField(self, selector[1], ast)
        if (!obj) throw new EvaluationError(`No such key: ${selector[2]}`, selector)
        return objectGet(obj, selector[2], self.objectTypesByConstructor)
      }
    }
  }
  throw new EvaluationError('has() invalid argument', ast)
...
// Helper: Extract element type from list<T> or map<K,V> types
function getIterableElementType(receiverType) {
  const listMatch = receiverType.match(/^list<(.+)>$/)
  if (listMatch) return listMatch[1]

  const mapMatch = receiverType.match(/^map<(.+),/)
  if (mapMatch) return mapMatch[1]

  return 'dyn'
}

// Helper: Validate first argument is an identifier and return variable name
function validateIdentifierArg(args, functionDesc) {
  ...
  throw new TypeError(`${functionDesc} requires first argument to be an identifier`, identArg)
}

function withVariableContext(checker, varName, varType, callback) {
  const prevVarType = checker.variables.get(varName)
  checker.variables.set(varName, varType)

  try {
    return callback()
  } finally {
    if (prevVarType !== undefined) checker.variables.set(varName, prevVarType)
    else checker.variables.delete(varName)
  }
}
...
function predicateTypeCheck(methodName) {
  return (checker, receiverType, args) => {
    ...
    const elementType = getIterableElementType(receiverType)

    return withVariableContext(checker, varName, elementType, () => {
      const predicateType = checker.check(args[1])
      if (!(predicateType === 'bool' || predicateType === 'dyn')) {
        throw new TypeError(`predicate must return bool, got '${predicateType}'`, args[1])
      }
      return 'bool'
    })
  }
...
function mapTypeCheck(checker, receiverType, args) {
  ...
  return withVariableContext(checker, varName, elementType, () => {
    if (args.length === 3) {
      const filterType = checker.check(args[1])
      if (!(filterType === 'bool' || filterType === 'dyn')) {
        throw new TypeError(
    ...
    const transformType = checker.check(args[args.length - 1])
    return transformType === 'dyn' ? 'list' : `list<${transformType}>`
  })
}
...
function filterTypeCheck(checker, receiverType, args) {
  ...
  return withVariableContext(checker, varName, elementType, () => {
    const filterType = checker.check(args[1])
    if (!(filterType === 'bool' || filterType === 'dyn')) {
      throw new TypeError(`${funcDesc} predicate must return bool, got '${filterType}'`, args[1])
    }
    return elementType === 'dyn' ? 'list' : `list<${elementType}>`
  })
}
...
functionOverload('map.map(ast, ast, ast): list', {handler: filterMapMacro, typeCheck: mapTypeCheck})

functionOverload('list.filter(ast, ast): list', {handler: filterMacro, typeCheck: filterTypeCheck})
...
function objectGet(obj, key, objectTypesByConstructor) {
  if (typeof obj !== 'object' || obj === null) return

  const type = objectTypesByConstructor.get(obj.constructor)
  if (!type) return
  ...
  switch (type.name) {
    ...
    default:
      ...
      if (!fields) return Object.hasOwn(obj, key) ? obj[key] : undefined
