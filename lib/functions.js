...
function withVariableContext(checker, varName, varType, check) {
  const prevVarType = checker.ctx.getType(varName)
  checker.ctx.set(varName, varType)

  try {
    return check()
  } finally {
    if (prevVarType !== undefined) checker.ctx.set(varName, prevVarType)
    else checker.ctx.delete(varName)
  }
}
