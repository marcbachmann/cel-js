...
function hasNestedField(self, ast) {
  ...
  if (typeof ast !== 'object' || ast === null || ast[0] === 'list' || ast[0] === 'map') {
...
function filterMapMacro(receiver, identifier, filter, predicate) {
  const evaluator = this.predicateEvaluator(receiver, 'map', identifier)
  const {items, predicateVariable, ctx} = evaluator
  const results = []
  for (let i = 0; i < items.length; i++) {
    ctx[predicateVariable] = items[i]
    switch (evaluator.eval(filter)) {
      case false:
        continue
      case true:
        results.push(evaluator.eval(predicate))
        continue
      default:
