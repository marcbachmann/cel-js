...
export class FunctionDeclaration {
  constructor({name, receiverType, argTypes, returnType, handler, typeCheck = null}) {
    ...
    if (`${this.receiverType}` === 'list<dyn>') this.receiverType = 'list'
    else if (`${this.receiverType}` === 'map<dyn, dyn>') this.receiverType = 'map'
...
export class Registry {
  ...
  registerFunctionOverload(s, handlerOrOptions) {
    ...
    for (const argType of dec.argTypes) {
      let typeName = argType.name
      if (typeName === 'list<dyn>') typeName = 'list'
      else if (typeName === 'map<dyn, dyn>') typeName = 'map'
      fnTarget = fnTarget[typeName] ??= Object.create(null)
    }

    fnTarget.macro = dec.macro
    fnTarget.argTypes = dec.argTypes
    fnTarget.handler = dec.handler
    fnTarget.returnType = dec.returnType
    fnTarget.signature = dec.signature
    if (dec.typeCheck) fnTarget.typeCheck = dec.typeCheck
