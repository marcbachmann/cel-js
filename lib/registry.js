...
class FunctionsByReceiver {
  ...
  #matchesArgs(fn, argTypes) {
    for (let i = 0; i < argTypes.length; i++) {
      ...
      if (declType.kind === 'dyn' && runtimeType === declType.valueType) continue
...
export class Registry {
  ...
  getFunctionCandidates(receiverType, functionName, argCount) {
    ...
    for (const [, dec] of this.#typeDeclarations) {
      ...
      if (receiverType && receiverType !== dynType && receiverType.kind !== dec.receiverType.kind)
        continue
  ...
  getVerifiedType(typeStr, assert) {
