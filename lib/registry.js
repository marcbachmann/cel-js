...
export class TypeDeclaration {
  ...
  #getMessageField(obj, key, ast, ev) {
    ...
    if (!type) return
    if (!type.fields) return hasOwn(obj, key) ? obj[key] : undefined
  ...
  #matches(s, o) {
    switch (s.kind) {
      ...
      case 'schema':
        // Schema types match maps at runtime (plain objects)
        return o.kind === 'map' || o.kind === 'schema'
      default:
...
export class Registry {
  ...
  registerVariable(name, type) {
    ...
    // Handle object schema: {fieldName: 'type', nested: {field: 'type'}}
    if (type !== null && typeof type === 'object' && !(type instanceof TypeDeclaration)) {
      type = this.#createSchemaType(name, type)
  ...
  #createSchemaType(name, schema) {
    ...
    for (const key of objKeys(schema)) {
      ...
      if (typeof fieldDef === 'string') {
        ...
        fields[key] = this.getType(fieldDef)
      } else if (fieldDef !== null && typeof fieldDef === 'object') {
        // Nested schema: {profile: {name: 'string'}}
        fields[key] = this.#createSchemaType(`${name}.${key}`, fieldDef)
      } else {
        throw new Error(`Invalid field definition for '${name}.${key}'`)
      }
    }

    // Create anonymous schema type with fields
    const typeName = `$${name}`
    const typeDecl = new TypeDeclaration({kind: 'schema', name: typeName, type: typeName})

    this.objectTypes.set(typeName, {
      name: typeName,
      type: typeDecl,
      fields
    })

    this.#typeDeclarations.set(typeName, typeDecl)
    return typeDecl
  }
  ...
  registerType(typename, _d, withoutDynRegistration) {
    if (typeof typename !== 'string' || typename.length < 2 || RESERVED.has(typename)) {
