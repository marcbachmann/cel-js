...
function createMacroChecker(msg, macro) {
  if (macro.callAst) return (chk, ctx) => chk.check(macro.callAst, ctx)
  if (macro.typeCheck) return (chk, ctx) => macro.typeCheck(chk, macro, ctx)
  throw new Error(`${msg} have a .callAst property or .typeCheck(checker, macro, ctx) method.`)
}

function createMacroEvaluator(msg, macro) {
  if (macro.callAst) return (ev, ctx) => ev.eval(macro.callAst, ctx)
  if (macro.evaluate) return (ev, ctx) => macro.evaluate(ev, macro, ctx)
  throw new Error(`${msg} have a .callAst property or .evaluate(evaluator, macro, ctx) method.`)
}
...
function wrapMacroExpander(name, handler) {
  ...
  return function macroExpander(opts) {
    ...
    return {
      check: createMacroChecker(p, macro),
      eval: createMacroEvaluator(p, macro)
    }
  }
}
...
export class Registry {
  ...
  #getCandidates(useReceiver, functionName, argCount) {
    let cache = this.#functionsCache.get(useReceiver)
    cache = cache.get(functionName) || cache.set(functionName, new Map()).get(functionName)
    return cache.get(argCount) || cache.set(argCount, new FunctionCandidates(this)).get(argCount)
  }

  getFunctionCandidates(useReceiver, functionName, argCount) {
    const cached = this.#functionsCache.get(useReceiver)?.get(functionName)?.get(argCount)
    if (cached) return cached

    for (const [, dec] of this.#functionDeclarations) {
      this.#getCandidates(!!dec.receiverType, dec.name, dec.argTypes.length).add(dec)
    }
    return this.#getCandidates(useReceiver, functionName, argCount)
  }
  ...
  #parseTypeString(typeStr, requireKnownTypes = false) {
    ...
    if (match) {
      const type = this.#parseTypeString(match[1].trim(), requireKnownTypes).wrappedType
  ...
  findMacro(functionName, hasReceiver, argCount) {
    return (
      ...
      this.#cacheOverloadResult(
        ...
        functionName,
        argCount,
        this.getFunctionCandidates(hasReceiver, functionName, argCount).declarations.find(
          (d) => d.macro
        ) || false
      )
    )
  ...
  registerFunctionOverload(s, _opts) {
    ...
    this.#functionsCache.get(true).clear()
    this.#functionsCache.get(false).clear()
  }
...
export class RootContext {
  ...
  forkWithVariable(name, type, value) {
    return new OverlayContext(this, name, type, value)
  }
}
...
class OverlayContext {
  ...
  secondaryType
  secondaryValue
  constructor(parent, primaryName, primaryType, primaryValue) {
    this.#parent = parent
  ...
  setValues(p, s) {
    this.primaryValue = p
    this.secondaryValue = s
    return this
  }

  getValue(key) {
    if (this.secondaryName === key) return this.secondaryValue
    if (this.primaryName === key) return this.primaryValue
    return this.#parent.getValue(key)
  }

  getType(key) {
    if (this.secondaryName === key) return this.secondaryType
    if (this.primaryName === key) return this.primaryType
