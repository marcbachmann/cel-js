...
export class TypeDeclaration {
  ...
  #hasDyn(t = this) {
    ...
    if (t.kind === 'list') return this.#hasDyn(t.valueType)
    if (t.kind === 'map') return this.#hasDyn(t.keyType) || this.#hasDyn(t.valueType)
    return false
  }

  hasNoDynTypes() {
    return this.#noDynTypes ?? (this.#noDynTypes = !this.#hasDyn())
  }
...
export class Registry {
  ...
  #matchesOverload(decl, actualLeft, actualRight) {
    ...
    return this.#validateParameters(decl, actualLeft, actualRight)
  }
  ...
  #unwrapDynType(type) {
    return type.kind === 'dyn' && type.valueType ? type.valueType : type
  }

  #validateParameters(decl, leftType, rightType) {
    const operator = decl.operator

    if (operator === 'in') {
      return this.#validateInOperator(decl, leftType, rightType)
    }

    if (operator === '+' || operator === '==' || operator === '!=') {
      const resolvedLeft = this.#unwrapDynType(leftType)
      const resolvedRight = this.#unwrapDynType(rightType)

      if (resolvedLeft.kind === 'list' && resolvedRight.kind === 'list') {
        return resolvedLeft.matchesBoth(resolvedRight)
      }

      if (resolvedLeft.kind === 'map' && resolvedRight.kind === 'map') {
        return resolvedLeft.matchesBoth(resolvedRight)
      }
    }
  ...
  #validateInOperator(decl, leftType, rightType) {
    const resolvedRight = this.#unwrapDynType(rightType)
    if (rightType.kind === 'dyn' && !rightType.valueType) return true

    if (resolvedRight.kind === 'list' || resolvedRight.kind === 'map') {
      const elementType =
        resolvedRight.kind === 'list' ? resolvedRight.valueType : resolvedRight.keyType

      const narrowedLeft = this.#unwrapDynType(leftType)

      if (narrowedLeft.kind === 'dyn' && !narrowedLeft.valueType) return true
      if (rightType.kind === 'dyn') return true

      const declaredLeftIsSimple = !['map', 'list', 'message'].includes(decl.leftType.kind)
      if (leftType.kind === 'dyn' && declaredLeftIsSimple) return true

      return elementType.matchesBoth(narrowedLeft)
    }
