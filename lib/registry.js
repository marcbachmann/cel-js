...
class FunctionsByReceiver {
  ...
  #matchesArgs(fn, argTypes) {
    for (let i = 0; i < argTypes.length; i++) {
      ...
      if (declType.type === 'dyn' || runtimeType.type === 'dyn') continue
      if (declType !== runtimeType) return false
    }
    return true
  }
}
...
class FunctionCandidates {
  ...
  filterByReceiverType(receiverType) {
    ...
    let result = this.#byReceiverType.get(receiverType)
    if (result) return result

    // For generic types, also try matching with the base type
    // e.g., if looking for list<string>, also check list<dyn>
    if (receiverType.type === 'list') {
      result = this.#byReceiverType.get(PRIMITIVE_TYPE_DECLARATIONS.list)
      if (result) return result
    }

    if (receiverType.type === 'map') {
      result = this.#byReceiverType.get(PRIMITIVE_TYPE_DECLARATIONS.map)
      if (result) return result
    }
...
export class Registry {
  ...
  #parseTypeString(typeStr, requireKnownTypes = false) {
    ...
    if (this.#typeDeclarations.get(typeStr)) return this.#typeDeclarations.get(typeStr)
