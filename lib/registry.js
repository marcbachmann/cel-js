...
export function createRegistry(opts) {
  ...
  const {functions, overloads, objectTypes, objectTypesByConstructor} = opts
  ...
  function parseFunctionDeclaration(signature) {
    ...
    for (let i = 0; i < argTypes.length; i++) {
      if (typesForFunctions.has(argTypes[i]) || objectTypes.has(argTypes[i])) continue
  ...
  function registerFunctionOverload(s, handler) {
