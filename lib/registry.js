...
export class TypeDeclaration {
  ...
  #getListField(obj, key, ast, ev) {
    if (typeof key === 'number' || typeof key === 'bigint') {
      ...
      if (key < 0) {
        throw new EvaluationError(`No such key: index out of bounds, index ${key} < 0`, ast)
      }

      if (key >= obj.length) {
        throw new EvaluationError(
          `No such key: index out of bounds, index ${key} >= size ${obj.length}`,
          ast
        )
      }
    }
...
export class Registry {
  ...
  constructor({typeDeclarations, ...opts} = {}) {
    ...
    this.#typeDeclarations = new Map(typeDeclarations || Object.entries(celTypes))
  }
  ...
  get overloads() {
    ...
    for (const [, decl] of this.#typeDeclarations) {
      ...
      const op = (overloads[decl.operator] ??= new Map())

      if (decl.isUnary) {
        op.set(decl.leftType, {
          handler: decl.handler,
          returnType: decl.returnType
        })
      } else {
        const leftMap = op.get(decl.leftType) || op.set(decl.leftType, new Map()).get(decl.leftType)
  ...
  getVerifiedType(typeStr) {
    return this.#typeDeclarations.get(typeStr)
  }
