...
const _dyn = _createPrimitiveType('dyn')
const PRIMITIVE_TYPE_DECLARATIONS = Object.freeze({
  ...
  list: _createListType(_dyn),
  'list<dyn>': _createListType(_dyn),
  map: _createMapType(_dyn, _dyn),
  'map<dyn, dyn>': _createMapType(_dyn, _dyn)
})
...
export class Registry {
  ...
  registerType(typename, _d, withoutDynRegistration) {
    ...
    this.registerFunctionOverload(`dyn(${typename}): dyn`, (v) => v)
  ...
  #toCelFieldDeclaration(typename, fieldname, fielddecl, requireKnownTypes = false) {
    ...
    if (field.rule === 'repeated') {
      ...
      const valueType = this.#parseTypeString(valueTypeStr, requireKnownTypes)
      return this.#createListType(valueType)
    } else if (field.rule) {
      throw new Error(`${p} has unsupported rule: ${field.rule}`)
    }

    const typeDecl = this.#parseTypeString(field.type, requireKnownTypes)
  ...
  #normalizeFields(typename, fields) {
    ...
    for (const k of Object.keys(fields))
      all[k] = this.#toCelFieldDeclaration(typename, k, fields[k])
    return all
  }
  ...
  #parseFunctionDeclaration(signature) {
    ...
    let returnType
    try {
      returnType = this.getType(_returnType || 'dyn')
    } catch (e) {
    ...
    for (let i = 0; i < argTypes.length; i++) {
      ...
      if (typesForFunctions.has(argType) || this.#objectTypes.has(argType)) continue
  ...
  registerFunctionOverload(s, handlerOrOptions) {
    ...
    const dec = this.#parseFunctionDeclaration(s)

    let fnTarget = dec.receiverType
      ? (this.#functions[dec.receiverType] ??= Object.create(null))
      : this.#functions.standalone
    ...
    if (!fnTarget.returnType) {
      this.#functions.returnTypes[dec.name] = fnTarget.returnType = dec.returnType
    } else if (fnTarget.returnType.type !== dec.returnType.type) {
      this.#functions.returnTypes[dec.name] = fnTarget.returnType = PRIMITIVE_TYPE_DECLARATIONS.dyn
    }
